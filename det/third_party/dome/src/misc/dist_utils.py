...
def warp_loader(loader, shuffle=False):
    if is_dist_available_and_initialized():
        ...
        is_multicrop = hasattr(loader.dataset, 'crops_per_image')
        
        if is_multicrop:
            # Use MultiCropDistributedSampler for better crop distribution
            from ..data.dataset import MultiCropDistributedSampler
            sampler = MultiCropDistributedSampler(loader.dataset, shuffle=shuffle)
        else:
            # Use standard DistributedSampler
            sampler = DistributedSampler(loader.dataset, shuffle=shuffle)
            
            # Print dataset information for non-multicrop datasets
            rank = get_rank()
            world_size = get_world_size()
            total_samples = len(loader.dataset)
            samples_per_gpu = len(sampler)
            
            print(f"[Rank {rank}] DistributedSampler initialized:")
            print(f"  - Total images: {total_samples}")
            print(f"  - Samples per GPU: {samples_per_gpu}")
            print(f"  - Batch size: {loader.batch_size}")
            print(f"  - Total batches per GPU: {samples_per_gpu // loader.batch_size}")
        ...
        if is_multicrop:
            print(f"DataLoader initialized (non-distributed, MultiCrop):")
            print(f"  - Original images: {len(loader.dataset.dataset)}")
            print(f"  - Total crops: {total_samples}")
        else:
            print(f"DataLoader initialized (non-distributed):")
            print(f"  - Total images: {total_samples}")
        
        print(f"  - Batch size: {loader.batch_size}")
        print(f"  - Total batches: {total_samples // loader.batch_size}")
