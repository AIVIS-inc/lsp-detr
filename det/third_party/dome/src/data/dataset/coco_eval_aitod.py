...
class CustomCOCOeval_faster(COCOeval_faster):
    """Custom COCOeval_faster that excludes IoU=0.25 from summary output"""
    
    def summarize(self):
        """Override summarize to exclude IoU=0.25 metric"""
        ...
        def _summarizeDets():
            stats = np.zeros((13,))  # Reduced from 15 to 13 (removed IoU=0.25 and IoU=0.75)
            stats[0] = _summarize(1, maxDets=self.params.maxDets[2])
            # stats[1] = _summarize(1, iouThr=.25, maxDets=self.params.maxDets[2])  # REMOVED
            stats[1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2])
            # stats[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])  # REMOVED
            stats[2] = _summarize(1, areaRng='verytiny', maxDets=self.params.maxDets[2])
            ...
            stats[9] = _summarize(0, areaRng='verytiny', maxDets=self.params.maxDets[2])
        ...
        def _summarizeKps():
            # Use parent class method for keypoints (not used in this project)
            return super(CustomCOCOeval_faster, self).summarize()
        ...
        if iouType == 'segm' or iouType == 'bbox':
            ...
            # For keypoints, use parent class implementation
            super(CustomCOCOeval_faster, self).summarize()
            return
        else:
            raise Exception('Unknown iouType for summarize')
...
class AitodCocoEvaluator(object):
    def __init__(self, coco_gt, iou_types):
        ...
        for iou_type in iou_types:
            # Use custom evaluator that excludes IoU=0.25
            coco_eval = CustomCOCOeval_faster(
                coco_gt, iouType=iou_type, print_function=print, separate_eval=True
            )
            # Custom maxDets for tiny object detection
            coco_eval.params.maxDets = [500, 1000, 2000]
