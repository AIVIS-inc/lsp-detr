...
class AitodCOCOeval_faster(COCOeval_faster):
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            _count = 16 if self.lvis_style else 13  # Reduced by 2 (removed IoU=0.25 and IoU=0.75)
            stats = np.zeros((_count,))

            stats[0] = self._summarize(1, maxDets=self.params.maxDets[2])
            # stats[1] = self._summarize(1, iouThr=.25, maxDets=self.params.maxDets[2])  # Removed
            stats[1] = self._summarize(1, iouThr=.5, maxDets=self.params.maxDets[2])
            # stats[2] = self._summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])  # Removed
            ...
            if self.lvis_style:
                stats[13] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=0)  # APr
                stats[14] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=1)  # APc
                stats[15] = self._summarize(1, maxDets=self.params.maxDets[-1], freq_group_idx=2)  # APf
        ...
        self.stats = self.all_stats[:10]  # Reduced from 12 to 10 (removed IoU=0.25 and IoU=0.75)
