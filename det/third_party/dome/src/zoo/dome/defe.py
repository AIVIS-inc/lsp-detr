...
class LiteDeFE(nn.Module):
    def __init__(self, hidden_dim=256):
        ...
        self.defe = OptimizedDeFE(hidden_dim=hidden_dim)
        
        # Density head intermediate channels scaled proportionally
        intermediate_dim = max(hidden_dim // 2, 64)
        self.density_head = nn.Sequential(
            nn.Conv2d(hidden_dim, intermediate_dim, 3, padding=1),
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False),
            nn.Conv2d(intermediate_dim, 1, 1),  # 输出 [B,1,H,W]
            nn.Sigmoid()
        )
    ...
    def forward(self, features):
        # Get input feature size to determine target output size
        # features: [B, hidden_dim, H, W] where H,W = original_img_size / 8
        ...
        safe_density = torch.nan_to_num(density, nan=0.0, posinf=0.0, neginf=0.0)
        # Normalize per-batch to ensure batch-size independence
        # max_val shape: [B, 1, 1, 1] for broadcasting
        max_val = safe_density.view(safe_density.size(0), -1).max(dim=1, keepdim=True)[0]
        max_val = max_val.view(safe_density.size(0), 1, 1, 1)
        density = torch.where(max_val > 0, safe_density / max_val, safe_density)
