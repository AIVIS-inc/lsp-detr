...
# The Hungarian matcher (scipy linear_sum_assignment) dominates step time on dense
# images and is called once per decoder/encoder aux layer. Those calls are independent
# and scipy releases the GIL while solving, so run them on a thread pool. Set
# DOME_MATCH_THREADS=1 to fall back to the original sequential behavior.
_MATCH_POOL = None


def _get_match_pool():
    global _MATCH_POOL
    n = int(os.environ.get("DOME_MATCH_THREADS", "6"))
    if n <= 1:
        return None
    if _MATCH_POOL is None:
        _MATCH_POOL = ThreadPoolExecutor(max_workers=n)
    return _MATCH_POOL
...
class DomeCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if "aux_outputs" in outputs:
            # Last-layer match, decoder aux layers (+ pre_outputs), then encoder aux
            # layers. Every match is independent and dominated by scipy's
            # linear_sum_assignment (which releases the GIL), so solve them concurrently
            # while preserving order -- the result is identical to the sequential version.
            dec_aux = list(outputs["aux_outputs"]) + [outputs["pre_outputs"]]
            enc_aux = list(outputs["enc_aux_outputs"])
            to_match = [outputs_without_aux] + dec_aux + enc_aux
            pool = _get_match_pool()
            if pool is not None and len(to_match) > 1:
                # Worker threads do not inherit the main thread's current CUDA device
                # (torch.cuda.set_device is thread-local), so pin each worker to the
                # device of the tensors it operates on before matching.
                def _match_one(o):
            ...
            indices = matched[0]
            cached_indices = matched[1 : 1 + len(dec_aux)]
            cached_indices_enc = matched[1 + len(dec_aux):]
            indices_aux_list = matched[1:]
