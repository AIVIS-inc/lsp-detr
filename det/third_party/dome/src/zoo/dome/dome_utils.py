...
def weighting_function(reg_max, up, reg_scale, deploy=False):
    ...
    """
    if deploy:
        # Optimized: Keep as tensor (will be converted later if needed)
...
def distance2bbox(points, distance, reg_scale):
    ...
    scale = (points[..., 2:] / reg_scale).clamp(min=1e-6)
    x1 = points[..., 0] - (0.5 * reg_scale + distance[..., 0]) * scale[..., 0]
    y1 = points[..., 1] - (0.5 * reg_scale + distance[..., 1]) * scale[..., 1]
    x2 = points[..., 0] + (0.5 * reg_scale + distance[..., 2]) * scale[..., 0]
    y2 = points[..., 1] + (0.5 * reg_scale + distance[..., 3]) * scale[..., 1]
