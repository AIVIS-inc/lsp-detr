...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False):
        ...
        padding_mask = outputs.get("query_padding_mask")
        if padding_mask is not None:
            padding_mask = padding_mask.to(outputs["pred_logits"].device)
            assert padding_mask.shape[:2] == (bs, num_queries)
        ...
        if padding_mask is not None:
            out_prob = out_prob.masked_fill(padding_mask.unsqueeze(-1), 0.0)

        out_prob = out_prob.flatten(0, 1)

        out_bbox = outputs["pred_boxes"]
        if padding_mask is not None:
            out_bbox = out_bbox.masked_fill(padding_mask.unsqueeze(-1), 0.0)
        ...
        if padding_mask is not None:
            C = C.masked_fill(padding_mask.unsqueeze(-1), 1e6)
