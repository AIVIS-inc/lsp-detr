...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.use_defe:
            ...
            if self.defe_type == "light":
                self.DeFE = LiteDeFE(hidden_dim=self.hidden_dim)
            else:
    ...
    def adaptive_defe_filter(self, defe_feature, init_thresh=0.05, step=0.01, min_ratio=0.02):
        """
        自适应调整阈值直到每个样本找到有效区域
        Args:
            defe_feature: 置信度特征图 [B, 1, H, W]
            init_thresh: 初始阈值 (绝对阈值上限)
            step: 阈值调整步长
            min_ratio: 当绝对阈值失效时采用的最大值相对比例下限
        Returns:
        ...
        ratios = torch.linspace(1.0, min_ratio, steps=8, device=device)
        ...
        for b in range(B):
            ...
            while current_thresh >= 0:
                ...
                if mask.any():
                    ...
                    found = True
                    break
                current_thresh = round(current_thresh - step, 4)

            # 2) fall back to relative thresholds
            if not found:
                max_val = single_feat.max()
                if max_val > 0:
                    for ratio in ratios:
                        mask = single_feat > (max_val * ratio)
                        if mask.any():
                            final_mask[b:b+1] = mask
                            found = True
                            break

            # 3) final fallback: use argmax location
            if not found:
                idx = torch.argmax(single_feat.view(-1))
                final_mask[b:b+1].view(-1)[idx] = True
                print(f"Batch {b}: No valid region found, fallback to argmax enhancement")
    ...
    def forward(self, feats, img_inputs, targets=None):
        ...
        if self.use_defe:
            ...
            if targets is not None:
                self._latest_gt_counts = [len(t.get("labels", [])) for t in targets]
